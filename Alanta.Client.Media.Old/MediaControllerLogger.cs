...
{
	public class MediaControllerLogger : ReactiveObject
	{
		...
		public MediaControllerLogger(IVideoQualityController videoQualityController, MediaStatistics mediaStatistics = null)
		{
		...
		public double AverageFramePlayedTime
		{
			...
			set
			{
				...
				Deployment.Current.Dispatcher.BeginInvoke(() => this.RaisePropertyChanged(x => x.AverageFramePlayedTime));
			}
		}
		...
		public double AverageFrameRecordedTime
		{
			...
			set
			{
				...
				Deployment.Current.Dispatcher.BeginInvoke(() => this.RaisePropertyChanged(x => x.AverageFrameRecordedTime));
			}
		}
		...
		public void LogAudioFramePlayed()
		{
			...
			double averageTotalTime = totalElapsed.TotalMilliseconds / _audioFramesPlayed;
			_playingRateCounter.Update(AverageFramePlayedTime);
			if ((now - _lastPlayedLog).TotalSeconds > 10)
			{
				ClientLogger.Debug("MediaController: Total audio frames played={0}; recentElapsed={1}; time/entry={2:f3}; recent time/entry={3:f3}",
		...
		public void LogAudioFrameTransmitted(bool isSilent)
		{
			...
			double averageTotalTime = totalElapsed.TotalMilliseconds / _audioFramesRecorded;
			_recordingRateCounter.Update(AverageFrameRecordedTime);
			if ((now - _lastRecordedLog).TotalSeconds > 10)
			{
				ClientLogger.Debug("MediaController: Total audio frames sent={0}; recentElapsed={1}; time/entry={2:f3}; recent time/entry={3:f3}",
		...
		public void LogVideoPacketSent(ByteStream packetBuffer)
		{
			...
			{
				double msSinceLastReset = (now - _lastVideoResetTime).TotalMilliseconds;
				if (msSinceLastReset > 2000)
				{
					_lastVideoResetTime = now;
					float recentBitsPerSecond = (float)((_recentVideoBytesSent / (msSinceLastReset / 1000)) * 8) / 1024;
		...
		public void LogAudioFrameSet()
		{
			...
			if (++_audioFramesSet % 1000 == 0)
			{
				...
				double averageTotalTime = totalElapsed.TotalMilliseconds / _audioFramesSet;
				ClientLogger.Debug("Total audio frames set={0}; recentElapsed={1}; time/entry={2:f3}; recent time/entry={3:f3}",
		...
		public void LogAudioInputQueueStatus(Queue<ByteStream> queue)
		{
			if (queue.Count > 5)
			{
				ClientLogger.Debug("The audio input queue has {0} members; this is larger than we should probably have.", queue.Count);
			}
		}
