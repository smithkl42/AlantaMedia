...
{
    public partial class ParamsTest
    {
        ...
        private void InitializeCaptureSource()
        {
            if (_captureSource == null)
            {
                ...
                if (_captureSource.AudioCaptureDevice != null)
                {
                    ...
                    {
                        _captureSource.AudioCaptureDevice.AudioFrameSize = AudioFormat.Default.MillisecondsPerFrame;
                            // 20 milliseconds
                        _audioSink = new AudioSinkAdapter(_captureSource, null, MediaConfig.Default,
                            new TestMediaEnvironment(), AudioFormat.Default);
        ...
        private void btnPlaySource_Click(object sender, RoutedEventArgs e)
        {
            if (!_player.IsConnected)
            {
                ...
                _player.StartPlaying(_sourceFrames, () => { btnPlaySource.Content = playText; });
            }
            else
            {
                _player.StopPlaying();
        ...
        private void btnPlaySpeakers_Click(object sender, RoutedEventArgs e)
        {
            if (!_player.IsConnected)
            {
                ...
                _player.StartPlaying(_speakerFrames, () => { btnPlaySpeakers.Content = playText; });
            }
            else
            {
                _player.StopPlaying();
        ...
        private void btnStart_Click(object sender, RoutedEventArgs e)
        {
            if (_testRunner == null || !_testRunner.IsRunning)
            {
                try
                {
                    ...
                    var selectedEchoCanceller =
                        (EchoCancellerType) Enum.Parse(typeof (EchoCancellerType), (string) selection.Content, true);
                    if (chkLive.IsChecked == true)
                    {
                        _testRunner = new TestRunnerLive(selectedEchoCanceller, sourceAudioVisualizer,
                            speakersAudioVisualizer, cancelledAudioVisualizer);
                    }
                    else
                    {
                        ...
                        _testRunner = new TestRunnerFast(selectedEchoCanceller, sourceAudioVisualizer,
                            speakersAudioVisualizer, cancelledAudioVisualizer, _speakerFrames);
                    }
        ...
        private void btnPlayResults_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (!_player.IsConnected)
            {
                ...
                _player.StartPlaying(results.Cancelled, () => { src.Content = playText; });
            }
            else
            {
                _player.StopPlaying();
        ...
        private void btnExport_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                {
                    const string header =
                        "ExpectedLatency,FilterLengthInMs,FilterLengthInSamples,RecordedVolumeSD,RecordedVolumeAverage,RecordedRMS,CancelledVolumeSD,CancelledVolumeAverage,CancelledRMS,VolumeChangePercent,VolumeChangeDecibels\r\n";
                    _testRunner.Results.SaveToCsv(header,
