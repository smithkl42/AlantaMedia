...
namespace Alanta.Client.Ui.Controls.AudioVisualizer
{
    public class FourierTransform
    {
        ...
        public static int[] MeterFrequencies = new int[FREQUENCYSLOTCOUNT] { 30, 55, 80, 120, 155, 195, 250, 375, 500, 750, 1000, 1500, 2000, 3000, 4000, 6000, 8000, 12000, 16000 };
        ...
        {
            MeterFrequencies = meterFrequencies;
        ...
        private static byte[] GetPeaksForChannel(double[] normalizedArray, int sampleFrequency)
        {
            ...
            var peaks = new byte[_frequencySlotCount];
            // update meter
            int centerFreq = (sampleFrequency / 2);
        	for (int i = 0; i < _frequencySlotCount; ++i)
            {
            	byte peak;
            	if (MeterFrequencies[i] > centerFreq)
                {
                    peak = 0;
                }
                else
                {
                    int index = MeterFrequencies[i] * normalizedArray.Length / sampleFrequency;
